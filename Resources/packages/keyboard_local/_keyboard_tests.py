...
def make_event(event_type, name, scan_code=None, time=0):
    return KeyboardEvent(
        ...
        scan_code=scan_code or dummy_keys[name][0][0],
        name=name,
        time=time,
    )
...
keyboard._os_keyboard.press = lambda scan_code: send_instant_event(
    make_event(KEY_DOWN, None, scan_code)
)
keyboard._os_keyboard.release = lambda scan_code: send_instant_event(
    make_event(KEY_UP, None, scan_code)
)
keyboard._os_keyboard.type_unicode = lambda char: output_events.append(
    KeyboardEvent(event_type=KEY_DOWN, scan_code=999, name=char)
)
...
class TestKeyboard(unittest.TestCase):
    ...
    def do(self, manual_events, expected=None):
        ...
        if expected is not None:
            to_names = lambda es: "+".join(
                ("d" if e.event_type == KEY_DOWN else "u") + "_" + str(e.scan_code)
                for e in es
            )
    ...
    def test_parse_hotkey_keys(self):
        self.assertEqual(
            keyboard.parse_hotkey("left shift + a"),
            (
                (
                    (5,),
                    (1,),
    ...
    def test_parse_hotkey_steps(self):
        self.assertEqual(
            keyboard.parse_hotkey("a+b, b+c"), (((1,), (2,)), ((2,), (3,)))
        )
    ...
    def test_parse_hotkey_example(self):
        ...
        self.assertEqual(
            keyboard.parse_hotkey("alt+shift+a, alt+b, c"),
    ...
    def test_on_press_nonblocking(self):
        keyboard.on_press(
            lambda e: self.assertEqual(e.name, "a")
            and self.assertEqual(e.event_type, KEY_DOWN)
        )
        self.do(d_a + u_a)
    ...
    def test_on_release(self):
        keyboard.on_release(
            lambda e: self.assertEqual(e.name, "a")
            and self.assertEqual(e.event_type, KEY_UP)
        )
        self.do(d_a + u_a)
    ...
    def test_write_unicode_explicit(self):
        ...
        self.do(
            [],
            [
                KeyboardEvent(event_type=KEY_DOWN, scan_code=999, name="a"),
                KeyboardEvent(event_type=KEY_DOWN, scan_code=999, name="b"),
            ],
        )
    ...
    def test_write_unicode_fallback(self):
        ...
        self.do(
            [],
            [KeyboardEvent(event_type=KEY_DOWN, scan_code=999, name="á")] + d_b + u_b,
        )
    ...
    def test_get_hotkey_name_modifiers(self):
        self.assertEqual(
            keyboard.get_hotkey_name(["a", "shift", "ctrl"]), "ctrl+shift+a"
        )
    ...
    def test_get_hotkey_name_multiple(self):
        self.assertEqual(
            keyboard.get_hotkey_name(["ctrl", "b", "!", "a"]), "ctrl+!+a+b"
        )
    ...
    def test_add_hotkey_single_step_suppress_args_allow(self):
        ...
        keyboard.add_hotkey(
            "a",
            lambda a: self.assertIs(a, arg) or trigger() or True,
            args=(arg,),
            suppress=True,
        )
        self.do(d_a, triggered_event + d_a)
    ...
    def test_add_hotkey_single_step_suppress_removed(self):
        ...
        self.assertEqual(
            keyboard._listener.filtered_modifiers[dummy_keys["left ctrl"][0][0]], 0
        )
    ...
    def test_add_hotkey_single_step_suppress_with_modifiers_fail_unrelated_modifier(
        self,
    ):
        keyboard.add_hotkey("ctrl+shift+a", trigger, suppress=True)
    ...
    def test_add_hotkey_single_step_suppress_with_modifiers_unrelated_key(self):
        keyboard.add_hotkey("ctrl+shift+a", trigger, suppress=True)
        self.do(
            d_ctrl + d_shift + du_b + d_a, d_shift + d_ctrl + du_b + triggered_event
        )

    def test_add_hotkey_single_step_suppress_with_modifiers_release(self):
        keyboard.add_hotkey("ctrl+shift+a", trigger, suppress=True)
        self.do(
            d_ctrl + d_shift + du_b + d_a + u_ctrl + u_shift,
            d_shift + d_ctrl + du_b + triggered_event + u_ctrl + u_shift,
        )
    ...
    def test_add_hotkey_single_step_suppress_with_modifiers_repeated(self):
        ...
        self.do(
            d_ctrl + du_a + du_b + du_a,
            triggered_event + d_ctrl + du_b + triggered_event,
        )
    ...
    def test_add_hotkey_single_step_suppress_with_modifier_superset_release(self):
        keyboard.add_hotkey("ctrl+a", trigger, suppress=True, trigger_on_release=True)
        self.do(
            d_ctrl + d_shift + du_a + u_shift + u_ctrl,
            d_ctrl + d_shift + du_a + u_shift + u_ctrl,
        )
    ...
    def test_parse_hotkey_combinations_single_modifier(self):
        self.assertEqual(
            keyboard.parse_hotkey_combinations("shift+a"), (((1, 5), (1, 6)),)
        )

    def test_parse_hotkey_combinations_single_modifiers(self):
        self.assertEqual(
            keyboard.parse_hotkey_combinations("shift+ctrl+a"),
            (((1, 5, 7), (1, 6, 7)),),
        )
    ...
    def test_parse_hotkey_combinations_multi_modifier(self):
        self.assertEqual(
            keyboard.parse_hotkey_combinations("shift+a, b"),
            (((1, 5), (1, 6)), ((2,),)),
        )
    ...
    def test_add_hotkey_multi_step_suppress_replays(self):
        keyboard.add_hotkey("a, b, c", trigger, suppress=True)
        self.do(
            du_a + du_b + du_a + du_b + du_space, du_a + du_b + du_a + du_b + du_space
        )
