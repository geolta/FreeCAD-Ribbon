...
SETUP_PACKAGES = ["setuptools-lint >=0.5", "sphinx >=1.3.1", "twine >=4.0"]
...
EXTRA_PACKAGES = {
    ...
    ':"linux" in sys_platform': ["evdev >= 1.3", "python-xlib >= 0.17"],
...
INFO = {}
with open(
    os.path.join(os.path.dirname(__file__), "lib", MAIN_PACKAGE_NAME, "_info.py"), "rb"
) as f:
...
try:
    ...
    with open(
        os.path.join(
            os.path.dirname(__file__), os.path.join("docs", "mouse-usage.rst")
        ),
        "rb",
    ) as f:
        README += "\n\n" + f.read().decode("utf-8")
    ...
    with open(
        os.path.join(
            os.path.dirname(__file__), os.path.join("docs", "keyboard-usage.rst")
        ),
        "rb",
    ) as f:
        README += "\n\n" + f.read().decode("utf-8")
...
try:
    with open(os.path.join(os.path.dirname(__file__), "CHANGES.rst"), "rb") as f:
...
setuptools.setup(
    ...
    url=PACKAGE_URL,
    packages=setuptools.find_packages(os.path.join(os.path.dirname(__file__), "lib")),
    package_dir={"": "lib"},
    zip_safe=True,
    test_suite="tests",
    license="LGPLv3",
