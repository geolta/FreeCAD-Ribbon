...
class KeyboardControllerTest(EventTest):
    ...
    def test_shift_pressed(self):
        ...
        for key in (
            ...
            self.controller.release(key)
            self.assertFalse(
                self.controller.shift_pressed, "shift_pressed was incorrectly set"
            )
    ...
    def test_type_nonascii(self):
        ...
        self.assert_input("Failed to type Spanish string", "Teclado (informática)")
        self.assert_input("Failed to type Russian string", "Компьютерная клавиатура")
    ...
    def test_controller_events(self):
        ...
        with self.assert_event(
            "Failed to send press", on_press=lambda k: getattr(k, "char", None) == "a"
        ):
