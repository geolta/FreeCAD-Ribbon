...
class MouseControllerTest(EventTest):
    ...
    def test_release(self):
        ...
        for b in (pynput.mouse.Button.left, pynput.mouse.Button.right):
            ...
            with self.assert_event(
                ...
                on_click=lambda x, y, button, pressed: button == b and not pressed,
            ):
                self.controller.release(b)
    ...
    def test_click(self):
        ...
        for b in (pynput.mouse.Button.left, pynput.mouse.Button.right):
            ...
            with self.assert_event("Failed to send click events", on_click=on_click):
                self.controller.click(b)

    def test_scroll_up(self):
        """Tests that scrolling up works"""
        with self.assert_event(
            "Failed to send scroll up event", on_scroll=lambda x, y, dx, dy: dy > 0
        ):
    ...
    def test_scroll_down(self):
        ...
        with self.assert_event(
            "Failed to send scroll down event", on_scroll=lambda x, y, dx, dy: dy < 0
        ):
            self.controller.scroll(0, -1)
