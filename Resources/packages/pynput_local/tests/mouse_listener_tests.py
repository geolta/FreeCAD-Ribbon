...
class MouseListenerTest(EventTest):
    ...
    def test_move(self):
        ...
        self.assert_cumulative("Failed to register movement", on_move=lambda a, b: True)

    def test_left(self):
        """Tests that move left events are emitted correctly"""
        self.notify("Move mouse pointer left")
        self.assert_cumulative(
            "Failed to register movement", on_move=lambda a, b: b[0] < a[0]
        )
    ...
    def test_up(self):
        ...
        self.assert_cumulative(
            "Failed to register movement", on_move=lambda a, b: b[1] < a[1]
        )
    ...
    def test_scroll_up(self):
        ...
        self.assert_stop(
            "No scroll up registered", on_scroll=lambda x, y, dx, dy: not (dy > 0)
        )
    ...
    def test_scroll_down(self):
        ...
        self.assert_stop(
            "No scroll down registered", on_scroll=lambda x, y, dx, dy: not (dy < 0)
        )
    ...
    def test_reraise(self):
        ...
        with self.assertRaises(MyException):
            with pynput.mouse.Listener(on_click=on_click) as l:
    ...
    def test_events(self):
        ...
        with Events() as events:
            ...
            for event in events:
                if isinstance(event, Events.Click) and event.button == Button.left:
                    break

            self.notify("Press the right mouse button")
            for event in events:
                if isinstance(event, Events.Click) and event.button == Button.right:
                    break
